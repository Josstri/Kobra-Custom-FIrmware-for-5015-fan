...
// Respond to panel request for 4 files starting at index
        ...
        *p_u8 = data_buf[1];
        ...
        else if (control_index == REG_LCD_READY)
        {
          ...
          *p_u8 = data_buf[4];
          p_u8++;
          *p_u8 = data_buf[3];
          if ((control_value & 0x00FFFFFF) == 0x010072)
          { // startup last gif
          ...
          else if ((control_value & 0x00FFFFFF) == 0x010000)
          {                                            // startup first gif
  ...
  void DgusTFT::page2_handle(void)
  {
    ...
    case 5: // resume of outage(last power off)
      ...
      SERIAL_ECHOLNPAIR("printer_state: ", printer_state);
#endif
      if (lcd_txtbox_index > 0 && lcd_txtbox_index < 6)
      { // 1~5
    ...
    case 11: // txtbox 5 click
    {
      ...
      if (file_index < filenavigator.getFileNum())
      {
        ...
        if (lcd_txtbox_index_last && lcd_txtbox_index_last != lcd_txtbox_index)
        { // 1~5
  ...
  void DgusTFT::page3_handle(void)
  {
    ...
    case 1: // return
    {
      ...
      { // only is idle status can return
        ChangePageOfTFT(PAGE_FILE);
      }
    }
    break;
